...
def clic_boton(fila, columna, event, tablero, botones, contador):
    ...
    if event == '<Button-1>':  # Clic izquierdo
        ...
        if botones[fila][columna]['text'] == ' ' and contador_bandera < minas:
            agregar_banderin(fila, columna, botones)
            contador_bandera += 1
        elif botones[fila][columna]['text'] == '🚩':
            eliminar_banderin(fila, columna, botones)
            contador_bandera -= 1
        contador.config(text=f"Contador: {contador_bandera}/{minas}")


def agregar_banderin(fila, columna, botones):
    botones[fila][columna].config(text='🚩', fg='red')

def eliminar_banderin(fila, columna, botones):
